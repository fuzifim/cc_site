<nav class="navbar navbar-default headerbar mb5">
	<div class="container-fluid">
	  <div class="navbar-header">
		...
		<a class="navbar-brand logo_header" href="{{route('channel.home',$channel['domainPrimary'])}}"><img class="" id="logoChannel" src="{{asset('assets/img/logo-red-white.svg')}}" alt="{!!$channel['info']->channel_name!!}" title="{!!$channel['info']->channel_name!!}"></a>
	  </div>
		<div id="navbarSearch" class="collapse" aria-expanded="false" style="height: 1px;">
		  
		</div>
	  <div id="navbar" class="navbar-collapse collapse" aria-expanded="false" style="height: 1px;">
		<div id="insertFormSearch">
			<div class="col-sm-3 col-md-3" id="groupSearchForm" itemscope itemtype="http://schema.org/WebSite"> 
				<meta itemprop="url" content="{{route('channel.home',$channel['domainPrimary'])}}"/>
				<form itemprop="potentialAction" itemscope itemtype="http://schema.org/SearchAction" id="searchform" class="navbar-form searchform" action="{{route('search.query',$channel['domainPrimary'])}}" method="get">
					<meta itemprop="target" content="{{route('search.query',array($channel['domainPrimary']))}}?v={v}"/>
					<div class="input-group formSearchTop">
		...
		<ul class="nav navbar-nav navbar-right">
			<li><a href="{{route('channel.add',$channel['domainPrimary'])}}">Tạo website</a></li>
		...
		@if (Auth::check())
		  <li  class="dropdown">
			...
			<ul class="dropdown-menu">
				@role(['admin','manage'])
				<li><a href="{{route('post.add',$channel['domainPrimary'])}}" class=""><i class="glyphicon glyphicon-edit"></i><span class=""> Đăng bài</span></a></li>
				@endrole
		...
		@else 
			<li><a href="{{route('channel.login',$channel['domainPrimary'])}}" class=""><i class="glyphicon glyphicon-user"></i><span class=""> Đăng nhập</span></a></li>
		@endif
		</ul>
	  </div><!--/.nav-collapse -->
	</div><!--/.container-fluid -->
