					...
					@foreach($getPosts as $post)
						<div class="row media">
							<div class="col-md-3 col-xs-12">
								<a href="{{route('channel.slug',array($channel['domain']->domain,$post->getSlug->slug_value))}}" class="" target="_blank">
							...
							<div class="col-md-9 col-xs-12">
							  <h2 class="blog-title text-primary"><a href="{{route('channel.slug',array($channel['domain']->domain,$post->getSlug->slug_value))}}" target="_blank">{!!$post->posts_title!!}</a></h2>
                ...
                <div class="panel-body results-list">
					@foreach($getItems as $item)
						@if($item->type=='post')
							...
							<div class="row media">
								<div class="col-md-3 col-xs-12">
									<a href="{{route('channel.slug',array($domainPrimary,$post->getSlug->slug_value))}}" class="" target="_blank">
								...
								<div class="col-md-9 col-xs-12">
								  <h2 class="blog-title text-primary"><a href="{{route('channel.slug',array($domainPrimary,$post->getSlug->slug_value))}}" target="_blank">{!!$post->posts_title!!}</a></h2>
								  @if(strlen($post->posts_description)>5)<p>{{WebService::limit_string(strip_tags(html_entity_decode($post->posts_description),""), $limit = 200)}}</p>@endif
						...
						@elseif($item->type=='channel')
							...
							<div class="row media">
								<div class="col-md-12">
									<h2 class="blog-title"><a href="{{route('channel.home',$domainPrimary)}}" target="_blank">{!!$getChannel->channel_name!!}</a></h2>
								</div>
							</div>
						@elseif($item->type=='company')
							...
							<div class="row media">
								<div class="col-md-12">
									<h2 class="blog-title"><a href="{{route('company.view.slug',array(config('app.url'),$getCompany->id.'-'.Str::slug($getCompany->company_name)))}}" target="_blank">{!!$getCompany->company_name!!}</a></h2>
								</div>
							</div>
						@endif
					@endforeach
