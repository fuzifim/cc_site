<?
	...
	Theme::setTitle(html_entity_decode($post->posts_title));
	Theme::setKeywords($key);
	Theme::setDescription(WebService::limit_string(strip_tags(html_entity_decode($post->posts_description),""), $limit = 200)); 
	Theme::setCanonical('https://post-'.$post->id.'.'.config('app.url'));
	if(count($post->gallery)>0){
		if($post->gallery[0]->media->media_storage=='youtube'){
			Theme::setImage('//img.youtube.com/vi/'.$post->gallery[0]->media->media_name.'/hqdefault.jpg'); 
			Theme::setVideo($post->gallery[0]->media->media_url); 
			Theme::setType('video'); 
			Theme::setUrl($post->gallery[0]->media->media_url); 
		}else if($post->gallery[0]->media->media_storage=='video'){
			Theme::setImage(config('app.link_media').$post->gallery[0]->media->media_path.'thumb/'.$post->gallery[0]->media->media_id_random.'.png');
		}else{
			if(!empty($post->gallery[0]->media->media_name)){
				Theme::setImage(config('app.link_media').$post->gallery[0]->media->media_path.'thumb/'.$post->gallery[0]->media->media_name);
			} 
			if(!empty($post->price->posts_attribute_value)){
				Theme::setType('product'); 
			}else{
				Theme::setType('article'); 
			}
			Theme::setUrl(route('channel.slug',array($channel['domainPrimary'],$post->getSlug->slug_value))); 
		}
	}
			...
			<div class="@if($options!=null && !empty($options->viewFullScreen) && $options->viewFullScreen=='checked') col-lg-12 col-md-12 col-sm-12 col-xs-12 @else col-lg-8 col-md-8 col-sm-12 col-xs-12 @endif" itemscope @if(!empty($post->price->posts_attribute_value)) itemtype="http://schema.org/Product" @else itemtype="http://schema.org/Article" @endif>
				...
				<div class="panel panel-default">
					<div class="panel-body content-show">
						<div class="mb5">
							<h1 class="panel-title"><strong itemprop="name">{!!$post->posts_title!!}</strong></h1>@if($channel['security']==true || (Auth::check() && Auth::user()->id==$post->author->user->id)) <a class="text-danger" href="{{route('post.edit',array($channel['domainPrimary'],$post->id))}}"><i class="glyphicon glyphicon-edit"></i> Sửa</a> @endif
						</div>
				...
				<div class="form-group">
					...
					<a class="btn btn-primary siteLink" href="javascript:void(0);" data-url='{{json_encode("https://www.facebook.com/sharer/sharer.php?u=https://post-".$post->id.".".config("app.url"))}}'><span class="fa fa-facebook"></span> Share on Facebook</a> 
					<a class="btn btn-info siteLink" href="javascript:void(0);" data-url='{{json_encode("https://twitter.com/share?url=https://post-".$post->id.".".config("app.url"))}}'><span class="fa fa-twitter"></span> Share on Twitter</a>
					</div>
				</div>
					...
					<div class="panel-body">
						...
						<div itemprop="seller" itemscope="" itemtype="http://schema.org/Person" class="">
							@if($channel['info']->channel_parent_id==0)
								...
								<a class="btn btn-success btn-block" href="tel:{{$postContact->contactPhone}}"><h3 class="nopadding nomargin"><i class="glyphicon glyphicon-earphone"></i> {{$postContact->contactPhone}}</h3></a>
								@elseif(!empty($post->author->user->phone))
								<a class="btn btn-success btn-block" href="tel:{{$post->author->user->phone}}"><h3 class="nopadding nomargin"><i class="glyphicon glyphicon-earphone"></i> {{$post->author->user->phone}}</h3></a>
								@endif
							@else
				...
				@if(count($postSearch)>0)
					<div class="mt5">
						<div class="panel-heading"><div class="panel-title">Tin đăng liên quan</div></div>
						<?
							$idPost=array(); 
							$i=0; 
						?>
						...
						@endforeach 
						<?
							$getPost = Cache::store('file')->remember('post_getPost_search_'.$post->id, 5, function() use($idPost)
							{
				...
				@endif
				<div class="form-group">
					<div class="panel-heading"><div class="panel-title">Tin đăng mới</div></div>
					@if(count($postsNew)>0) 
						@foreach($postsNew->chunk(3) as $chunk)
						{!!Theme::partial('listPostChannel', array('chunk' => $chunk))!!}
						@endforeach
					@endif
				</div>
...
<?
	...
	Theme::asset()->writeScript('custom', '
		...
		$(".btnDelComment").click(function(){
			...
			$.ajax({
				url: "'.route("channel.comment.del",config('app.url')).'",
		...
		$("#panelComment").on("click",".addComment",function() {
			...
			$.ajax({
				url: "'.route("channel.comment.add",config('app.url')).'",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				type: "post",
				cache: false,
				contentType: false,
