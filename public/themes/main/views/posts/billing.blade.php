<?
	$channel['theme']->setTitle('Tin cần thanh toán');
	$channel['theme']->setKeywords('');
	$channel['theme']->setDescription('Danh sách tin cần thanh toán'); 
				...
				@if(count($posts)>0)
					<ul class="list-group listMember">
						...
						<li class="list-group-item">
							...
							?>
							<i class="glyphicon glyphicon-list-alt"></i> <span class="text-success">{{$post->posts_title}}</span>
							<p>
							...
							<div class="form-group">
								<a href="#" class="btn btn-xs btn-success postOrder" data-id="{{$post->id}}"><i class="glyphicon glyphicon-usd"></i> Thanh toán</a> <a href="{{route('post.edit',array($domainPrimary,$post->id))}}" {{$target}} class="btn btn-xs btn-default">Sửa</a> <button type="button" class="btn btn-xs btn-danger postDelete" data-id="{{$post->id}}">Xóa</button>
							</div>
						</li>
						@endforeach
					</ul>
	...
	$channel['theme']->asset()->writeScript('custom','
		jQuery(document).ready(function(){
			...
			$(".postOrder").click(function () {
				...
				formData.append("cartType", "postOrder"); 
				$.ajax({
