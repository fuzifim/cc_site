...
{!!Theme::partial('headerbar', array('title' => 'Header'))!!}
   ...
   itemprop="item" href="{{route('channel.home',$channel['domainPrimary'])}}"><i class="flag flag-{{mb_strtolower($company->joinAddress[0]->address->joinRegion->region->iso)}}"></i> <span class="hidden-xs" itemprop="name">Cung Cấp</span></a></li> 
			...
			@if(!empty($company->joinAddress[0]->address->joinSubRegion->subregion->id))<li class="breadcrumb-item active" itemprop="itemListElement"><a itemprop="item" href="{{route('channel.slug',array($channel['domain']->domain,str_replace('/VN/','',$company->joinAddress[0]->address->joinSubRegion->subregion->SolrID)))}}"><span itemprop="name">{!!$company->joinAddress[0]->address->joinSubRegion->subregion->subregions_name!!}</span></a></li>@endif
			@if(!empty($company->joinAddress[0]->address->joinDistrict->district->id))<li class="breadcrumb-item active" itemprop="itemListElement"><a itemprop="item" href="{{route('channel.slug',array($channel['domain']->domain,str_replace('/VN/','',$company->joinAddress[0]->address->joinDistrict->district->SolrID)))}}"><span itemprop="name">{!!$company->joinAddress[0]->address->joinDistrict->district->district_name!!}</span></a></li>@endif
			@if(!empty($company->joinAddress[0]->address->joinWard->ward->id))<li class="breadcrumb-item active" itemprop="itemListElement"><a href="{{route('channel.slug',array($channel['domain']->domain,str_replace('/VN/','',$company->joinAddress[0]->address->joinWard->ward->SolrID)))}}"><span itemprop="name">{!!$company->joinAddress[0]->address->joinWard->ward->ward_name!!}</span></a></li>@endif
			@endif
			<li class="breadcrumb-item" itemprop="itemListElement"><span itemprop="name">{!!$company->company_name!!}</span></li>
		</ol>
		<div class="row-pad-5">
			<div class="col-md-8">
								...
								@foreach($company->joinChannel as $joinChannel)
									@if(count($joinChannel->channel->domainAll)>0)
										@foreach($joinChannel->channel->domainAll as $domainAll)
											<p><i class="glyphicon glyphicon-globe"></i> {{route('channel.home',$domainAll->domain->domain)}}</p>
										@endforeach
									@endif
								@endforeach
							@else
			...
			<div class="col-md-4">
				<div class="panel panel-default">
					<div class="panel-heading"><div class="panel-title">Công Ty Cùng Ngành</div></div>
					<div class="form-group">
						<script async src="//pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script>
						<!-- Ad News -->
						<ins class="adsbygoogle"
