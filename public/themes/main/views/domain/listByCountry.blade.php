                                ...
                                @if(!empty($item['attribute']['rank']))
                                    <p>
                                        ...
                                        @if(!empty($item['attribute']['country_code']))
                                            <span class="">Rank in <i class="flag flag-16 flag-{!! mb_strtolower($country->iso) !!}"></i> <a href="{!! route('domain.country.iso',array(config('app.url'),$item['attribute']['country_code'])) !!}">{!! $country->country !!}</a>@if(!empty($item['attribute']['rank_country'])): {!! Site::price($item['attribute']['rank_country']) !!}@endif
                                            </span>
                                        @endif
                                    </p>
                                @endif
                                @if(!empty($item['view']))<small><strong>Views: {!! $item['view'] !!}</strong></small><br>@endif
                                @if(!empty($item['title']))<span>{!! WebService::renameBlacklistWord(mb_substr($item['title'], 0, \App\Model\Mongo_Image::MAX_LENGTH_TITLE)) !!}</span>@endif
                            </li>
                            @endforeach
                        </ul>
