					...
					@foreach($getService->attributeAll as $attribute)
						<?
							...
							if(Session::has('channelPackge') && Session::get('channelPackge')==$attribute->id){
								$active='active'; 
								$checked='checked'; 
							}else if(!Session::has('channelPackge') && $attribute->id==19){
								$active='active'; 
								$checked='checked'; 
							}else{
								$active=''; 
							...
							<div class="list-group-item btn pricingPackge {{$active}}">
								...
								<div class="price-features">
									<ul class="list-group">
										...
										<li class="list-group-item list-group-item-info">@if($attributeJson->domain=='ok')<i class="fa fa-check text-success"></i> .com/ .net/ .com.vn/ .vn @else <i class="fa fa-check text-danger"></i> tên miền .cungcap.net @endif</li>		<li class="list-group-item list-group-item-info">@if($attributeJson->ssl=='ok')<i class="fa fa-check text-success"></i>@else<i class="fa fa-check text-danger"></i>@endif Chứng chỉ bảo mật SSL</li>
									</ul>
								</div>
