				...
				<div class="listItem">
					...
					<div class="panel panel-default row-pad-5">
						...
						@foreach($getSite as $site)
							<li class="list-group-item"><img width="16" height="16" alt="{{$site->domain}}" src=" //www.google.com/s2/favicons?domain={{$site->domain}}" class="lazy"> <a class="siteLink" data-url='{{json_encode(route("go.to.url",array(config("app.url"),urlencode("http://".$site->domain))))}}' href="http://{{$site->domain}}.{{config('app.url')}}" target="_blank">{{$site->domain}}</a> <small><span class="time-update"><i class="glyphicon glyphicon-time"></i> {!!WebService::time_request($site->updated_at,'en')!!}</span> - {{$site->view}} views</small>@if(!empty($site->title))<p>{{str_limit(strip_tags($site->title,''), 100)}}</p>@if(!empty($site->attribute['rank']))<p>Rank <span class="label label-primary"><i class="glyphicon glyphicon-globe"></i> {{Site::price($site->attribute['rank'])}}</span> @if(!empty($site->attribute['country_code'])&&!empty($site->attribute['rank_country']))
								<?
