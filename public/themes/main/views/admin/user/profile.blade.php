...
<?
	...
	if(!empty($user->joinAddress->address->id)){
		$userAddress=$user->joinAddress->address;
	}
	if(!empty($user->joinAddress->address->joinRegion->region->id)){
		$userRegion=$user->joinAddress->address->joinRegion->region;
	}
	if(!empty($user->joinAddress->address->joinSubRegion->subregion->id)){
		$userSubRegion=$user->joinAddress->address->joinSubRegion->subregion;
	}
	if(!empty($user->joinAddress->address->joinDistrict->district->id)){
		$userDistrict=$user->joinAddress->address->joinDistrict->district;
	}
	if(!empty($user->joinAddress->address->joinWard->ward->id)){
		$userWard=$user->joinAddress->address->joinWard->ward;
	}
					...
					<div class="panel panel-default">
						<div class="panel-body">
							...
							<div class="form-group">
								...
								<div class="input-group">
									<span class="input-group-addon" id="basic-addon1"><i class="flag flag-16 flag-@if(!empty($user->joinAddress->address->joinRegion->region->id)){!!mb_strtolower($user->joinAddress->address->joinRegion->region->iso)!!}@elseif(!empty($channel['region']->iso)){!! mb_strtolower($channel['region']->iso) !!}@endif"></i></span>
							...
							<div class="form-group">
								<div class="input-group">
									...
									<input type="text" name="userAddress" value="@if(!empty($userAddress->id)){!!$userAddress->address!!}@endif" class="form-control" placeholder="Địa chỉ đường, số nhà công ty, cửa hàng..." required />
								</div>
								<label class="error" for="userAddress"></label>
							</div>
							<div class="form-group row-pad-5">
								<div class="col-sm-6">
									<input type="hidden" name="idRegion" value="@if(!empty($userRegion->id)){!!$userRegion->id!!}@elseif(!empty($channel['region']->id)){{$channel['region']->id}}@endif">
									<input type="hidden" name="regionIso" value="@if(!empty($userRegion->iso)){!!mb_strtolower($userRegion->iso)!!}@elseif(!empty($channel['region']->iso)){{mb_strtolower($channel['region']->iso)}}@endif">
									<div class="addSelectRegion"></div>
									<div class="mb10"></div>
								</div>
								<div class="col-sm-6">
									<input type="hidden" name="idSubRegion" value="@if(!empty($userSubRegion->id)){!!$userSubRegion->id!!}@endif">
							...
							<div class="form-group row-pad-5">
								<div class="col-sm-6">
									<input type="hidden" name="idDistrict" value="@if(!empty($userDistrict->id)){!!$userDistrict->id!!}@endif">
									<div class="addSelectDistrict"></div>
									<div class="mb10"></div>
								</div>
								<div class="col-sm-6">
									<input type="hidden" name="idWard" value="@if(!empty($userWard->id)){!!$userWard->id!!}@endif">
...
<?
	if(!empty($userSubRegion->id)){
		$defaultSubRegionId=$userSubRegion->id;
	}else{
		$defaultSubRegionId=0;
	}
	if(!empty($userDistrict->id)){
		$defaultDistrictId=$userDistrict->id;
	}else{
		$defaultDistrictId=0;
	}
	if(!empty($userWard->id)){
		$defaultWardId=$userWard->id;
	}else{
		$defaultWardId=0;
	}
	$dependencies = array();
	$channel['theme']->asset()->writeScript('custom','
		...
		$(".groupResend").on("click",".btnResend",function() {
			...
			$.ajax({
				url: "'.route("channel.profile.resend",$channel["domainPrimary"]).'",
				type: "POST",
		...
		$("#myModal").on("click","#btnSavePassword",function() {
			...
			$.ajax({
				url: "'.route("channel.profile.changepass",$channel["domainPrimary"]).'",
				type: "POST",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
		...
		$("#joinStaff").click(function(){
			$.ajax({
				url: "'.route("channel.profile.joinstaff",$channel["domainPrimary"]).'",
				type: "POST",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
		...
		$("#profileBank").click(function(){
			...
			$.ajax({
				url: "'.route("channel.profile.bank",$channel["domainPrimary"]).'",
				type: "GET",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
		...
		$("#myModal").on("click","#btnSaveBank",function() {
			...
			$.ajax({
				url: "'.route("channel.profile.bank.request",$channel["domainPrimary"]).'",
				type: "post",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
		...
		$("#profileVoucher").click(function(){
			...
			$.ajax({
				url: "'.route("channel.profile.voucher",$channel["domainPrimary"]).'",
				type: "GET",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
		...
		$("#profileCustomers").click(function(){
			...
			$.ajax({
				url: "'.route("channel.profile.customers",$channel["domainPrimary"]).'",
				type: "GET",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
		...
		$("#profileSales").click(function(){
			...
			$.ajax({
				url: "'.route("channel.profile.sales",$channel["domainPrimary"]).'",
				type: "GET",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
		...
		$("#btnProfileSave").click(function(){
			...
			if(!$valid) {
				...
				$.ajax({
					url: "'.route("profile.save",$channel["domainPrimary"]).'",
					headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
					type: "POST",
					cache: false,
					contentType: false,
		...
		$("#changeAvata").bind("change", function(){
			...
			$.ajax({
				url: "'.route("channel.upload.file",$channel["domainPrimary"]).'",
				type: "post",
				headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
				cache: false,
				contentType: false,
				...
				success:function(result){
					...
					$.ajax({
						url: "'.route("profile.logo.change",$channel["domainPrimary"]).'",
						type: "post",
						headers: {"X-CSRF-TOKEN": $("meta[name=_token]").attr("content")},
						cache: false,
						contentType: false,
