			...
			<div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
				<div class="panel panel-default">
					...
					@if(!empty($channel['info']->channelService->name))
						<?
							if($channel['info']->channelService->id!=1 && $channel['info']->channel_date_end>\Carbon\Carbon::now()->addMonth(1)->format('Y-m-d H:i:s')){
								$listItem='list-group-item-success'; 
								$channelStatus=''; 
								$option='<button type="button" class="btn btn-xs btn-success reOrder" data-id="'.$channel['info']->id.'"><i class="glyphicon glyphicon-repeat"></i> Gia hạn</button>'; 
							}else if($channel['info']->channelService->id!=1 && $channel['info']->channel_date_end<\Carbon\Carbon::now()->format('Y-m-d H:i:s')){
								$listItem='list-group-item-danger'; 
								$channelStatus='<div class="alert alert-danger">Hết hạn</div>'; 
								$option='<button type="button" class="btn btn-xs btn-success reOrder" data-id="'.$channel['info']->id.'">Gia hạn</button>';
							}else if($channel['info']->channelService->id!=1 && \Carbon\Carbon::parse($channel['info']->channel_date_end)->format('Y-m-d H:i:s')<\Carbon\Carbon::now()->addMonth(1)->format('Y-m-d H:i:s')){
								$listItem='list-group-item-warning'; 
								$channelStatus='<div class="alert alert-danger"><strong><i class="glyphicon glyphicon-warning-sign"></i></strong> Sắp hết hạn</div>'; 
								$option='<button type="button" class="btn btn-xs btn-success reOrder" data-id="'.$channel['info']->id.'"><i class="glyphicon glyphicon-repeat"></i> Gia hạn</button>'; 
							}else{
								$listItem=''; 
								$channelStatus=''; 
						...
						<p><strong>Gói: {{$channel['info']->channelService->name}}</strong></p> 
									...
									+"<div class=\"form-group\">"
										+"<div class=\"list-group-item text-center btn packgeCheck\">"
											...
											+"<div class=\"list-group-item-text\">"
												...
												+"<p><i class=\"glyphicon glyphicon-cloud\"></i> "+parseInt($.parseJSON(item.attribute_value).limit_cloud).toLocaleString()+" MB</p>"
												+"<p><i class=\"glyphicon glyphicon-check\"></i> "+parseInt($.parseJSON(item.attribute_value).limit_post).toLocaleString()+" Bài viết</p>"
												+"<p><i class=\"glyphicon glyphicon-check\"></i> Bỏ quảng cáo</p>"
												+"<p><i class=\"glyphicon glyphicon-check\"></i> Tên miền riêng</p>"
											+"</div>"
											+"<input type=\"radio\" class=\"hidden\" value=\""+item.id+"\" name=\"channelPackge\">"
										+"</div>"
									+"</div>"
