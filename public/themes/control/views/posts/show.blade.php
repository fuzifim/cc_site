		...
		@else
		   ...
		   itemprop="item" href="{{route('channel.home',$channel['domainPrimary'])}}"><i class="fa fa-home"></i> <span class="hidden-xs" itemprop="name">Trang chủ</span></a></li> 
					@if(count($post->postsJoinCategory[0]->getCategory->parent)>0)
						...
						@endforeach
					@endif
					<li class="breadcrumb-item active" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"><a itemscope itemtype="http://schema.org/Thing"  itemprop="item" href="{{route('channel.slug',array($channel['domainPrimary'],$post->postsJoinCategory[0]->getCategory->getSlug->slug_value))}}"><span itemprop="name">{!!$post->postsJoinCategory[0]->getCategory->category_name!!}</span></a></li> 
				</ol> 
			@endif
		@endif
		<div class="row-pad-5 section-content postView">
			<div class="@if($options!=null && !empty($options->viewFullScreen) && $options->viewFullScreen=='checked') col-lg-12 col-md-12 col-sm-12 col-xs-12 @else col-lg-8 col-md-8 col-sm-12 col-xs-12 @endif" itemscope @if(!empty($post->price->posts_attribute_value)) itemtype="http://schema.org/Product" @else itemtype="http://schema.org/Article" @endif>
				...
				<div class="form-group">
					...
					<a class="btn btn-primary siteLink" href="javascript:void(0);" data-url='{{json_encode("https://www.facebook.com/sharer/sharer.php?u=".route('channel.slug',array($channel['domainPrimary'],$post->getSlug->slug_value)))}}'><span class="fa fa-facebook"></span> Share on Facebook</a> 
					<a class="btn btn-info siteLink" href="javascript:void(0);" data-url='{{json_encode("https://twitter.com/share?url=".route('channel.slug',array($channel['domainPrimary'],$post->getSlug->slug_value)))}}'><span class="fa fa-twitter"></span> Share on Twitter</a>
					</div>
				</div>
				@if(!empty($post->posts_description))
				<div class="panel panel-default">
					<div id="postDescription" class="panel-body postDescription postGallery table-responsive" itemprop="description">
						{!!str_replace("img.dakenh.net","img.cungcap.net",WebService::addNofollow(html_entity_decode($post->posts_description),$channel['domainPrimary'],true))!!}
					</div>
				</div>
				@endif
				@if(count($post->gallery)>0)
				...
				@if($channel['info']->channel_parent_id!=0)
					<div class="panel panel-info">
						...
						<div class="panel-body">
							@if(isset($channel['info']->companyJoin->company))<h5 class="subtitle mb5"><strong>{{$channel['info']->companyJoin->company->company_name}}</strong></h5>@endif
				...
				<div class="panel panel-default">
					...
					<div class="tab-content">
						<div id="panelComment" class="tab-pane fade active in">
							...
							<div class="form-group">
								<input type="text" name="commentName" class="form-control" placeholder="Tên của bạn" required>
							</div>
							<div class="form-group">
								<input type="text" name="commentPhone" class="form-control" placeholder="Số điện thoại" required>
							</div>
							<div class="form-group">
								<input type="text" name="commentEmail" class="form-control" placeholder="Địa chỉ Email" required>
							</div>
							<div class="form-group">
														...
														<div class="comment-box">
															<div class="comment-head">
																@if($commentJoinPost->comment->attribute->from=='user')
																	...
																	<h6 class="comment-name" itemprop="creator" itemscope itemtype="https://schema.org/Person"><a href="#" itemprop="name">{{$commentJoinPost->comment->author->user->name}}</a><span itemprop="commentTime" datetime="{!!Site::Date($commentJoinPost->comment->created_at)!!}">{!!WebService::time_request($commentJoinPost->comment->created_at)!!}</span>
																	@if(!empty($commentJoinPost->comment->name->attribute_value))<span>{!!$commentJoinPost->comment->name->attribute_value!!}</span>@endif 
																	@if(!empty($commentJoinPost->comment->phone->attribute_value))<span>{!!$commentJoinPost->comment->phone->attribute_value!!}</span>@endif 
																	@if(!empty($commentJoinPost->comment->email->attribute_value))<span>{!!$commentJoinPost->comment->email->attribute_value!!}</span>@endif
																	</h6>
																...
																@else
																	...
																	<h6 class="comment-name" itemprop="creator" itemscope itemtype="https://schema.org/Person"><a href="#" itemprop="name">{{$commentJoinPost->comment->author->attribute_value}}</a><span itemprop="commentTime" datetime="{!!Site::Date($commentJoinPost->comment->created_at)!!}">{!!WebService::time_request($commentJoinPost->comment->created_at)!!}</span>
																	@if(!empty($commentJoinPost->comment->name->attribute_value))<span>{!!$commentJoinPost->comment->name->attribute_value!!}</span>@endif 
																	@if(!empty($commentJoinPost->comment->phone->attribute_value))<span>{!!$commentJoinPost->comment->phone->attribute_value!!}</span>@endif 
																	@if(!empty($commentJoinPost->comment->email->attribute_value))<span>{!!$commentJoinPost->comment->email->attribute_value!!}</span>@endif
																	</h6>
			...
			<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
				...
				<div class="panel panel-default form-group">
					<div class="panel-heading"><i class="glyphicon glyphicon-list-alt"></i> Bài mới</div>
					<div class="panel-body">
						@foreach($postsNew as $postNew)
							@if(count($postNew->gallery)>0 && !empty($postNew->gallery[0]->media->media_url))
								<div class="row no-gutter">
									<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
										<a class="image" href="{{route('channel.slug',array($channel['domainPrimary'],$postNew->getSlug->slug_value))}}">
											@if($postNew->gallery[0]->media->media_storage=='youtube')
												<img src="//img.youtube.com/vi/{{$postNew->gallery[0]->media->media_name}}/default.jpg" class="img-responsive img-thumbnail lazy" alt="{!!$postNew->posts_title!!}" title="" >
											@elseif($postNew->gallery[0]->media->media_storage=='video')
											<div class="groupThumb" style="position:relative;">
												<span class="btnPlayVideoClickSmall"><i class="glyphicon glyphicon-play"></i></span>
												<img itemprop="image" class="img-responsive imgThumb lazy" alt="{{$postNew->posts_title}}" src="{{config('app.link_media').$postNew->gallery[0]->media->media_path.'thumb/'.$postNew->gallery[0]->media->media_id_random.'.png'}}"/>
											</div>
											@elseif($postNew->gallery[0]->media->media_storage=='files')
											<img src="{!!asset('assets/img/file.jpg')!!}" class="img-responsive imgThumb lazy" alt="{!!$postNew->posts_title!!}" title="" >
											@else
												<img src="{{config('app.link_media').$postNew->gallery[0]->media->media_path.'xs/'.$postNew->gallery[0]->media->media_name}}" class="img-responsive img-thumbnail" alt="" title="" >
											@endif
										</a>
									</div>
									<div class="col-lg-9 col-md-9 col-sm-9 col-xs-9">
										<h5 class="postTitle"><a class="title" href="{{route('channel.slug',array($channel['domainPrimary'],$postNew->getSlug->slug_value))}}">{!!$postNew->posts_title!!}</a></h5>
										<small><span>{{$postNew->posts_view}} lượt xem</span></small>
									</div>
								</div>
							@endif
						@endforeach
...
<?
	...
	$channel['theme']->asset()->writeScript('custom', '
		...
		$("#panelComment").on("click",".addComment",function() {
			...
			formData.append("commentName", $("#panelComment input[name=commentName]").val()); 
			formData.append("commentPhone", $("#panelComment input[name=commentPhone]").val()); 
			formData.append("commentEmail", $("#panelComment input[name=commentEmail]").val()); 
			$.ajax({
