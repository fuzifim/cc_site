<?
	if(!empty($post->id)){
		...
		$title='Đăng bài'; 
		$description='Đăng bài lên '.$channel['info']->channel_name; 
	}
...
{!!Theme::partial('headerbar', array('title' => 'Header'))!!}
	...
	<div class="contentpanel">
		...
		@endif
		@if(empty($post->id))
		@if($channel['totalSize']>=$channel['limitSize'] || $channel['totalPosts']>=$channel['limitPosts'])
			<div class="form-group text-center">
				<div class="alert alert-danger">
		...
		<form id="postForm" class="form-group" style="position:relative; ">
			...
			<div class="form-group">
				<input id="item-title" name="postTitle" style="font-size:18px;" value="@if(!empty($post->posts_title)){{$post->posts_title}}@endif" type="text" class="form-control title-post-edit" placeholder="Nhập tiêu đề bài đăng..." required>
								...
								@foreach ($post->gallery as $image)
									...
									@elseif($image->media->media_storage == "youtube")
										<div class="col-lg-2 col-md-2 col-sm-3 col-xs-4 itemFile" style="position:relative;">
											<a href="" class="delMediaData" style="position:absolute; bottom:0px; right:0px;" data-id="{{$image->media->id}}"><span class="label label-danger"><i class="fa fa-trash-o"></i> xóa</span></a>
