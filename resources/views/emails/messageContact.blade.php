...
<div style="@if(!empty($channelColor->channelTitle)) background:{{$channelColor->channelTitle}}; @else background:#337ab7; @endif padding:10px;">
	...
	<div style="background:#ffffff;border:solid 1px #005c73;padding:10px;">
		<p>Xin chào <strong>{!!$channel->channel_name!!}</strong>, Bạn có một yêu cầu liên hệ từ <strong>{!!$messageArray->name!!}</strong></p>
		<strong>Email:</strong> {!!$messageArray->email!!}<br> 
		<strong>Điện thoại:</strong> {!!$messageArray->phone!!}<br> 
		<strong>Vào lúc:</strong> {!!$msgValue->created_at!!}<br> 
		<div style="border:solid 1px #dadada;margin-top:10px;padding:10px;">
			@if(!empty($messageArray->title))<p><strong>{!!$messageArray->title!!}</strong></p>@endif
			@if(!empty($messageArray->content))
				<p>{!!$messageArray->content!!}</p>
			@endif
		</div>
		<p><small>Bạn có thể liên hệ với <strong>{!!$messageArray->name!!}</strong> bằng cách trả lời thư này hoặc liên hệ trực tiếp theo các thông tin trên. </small></p>
	</div>
