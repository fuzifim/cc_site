...
<div style="@if(!empty($channelColor->channelTitle)) background:{{$channelColor->channelTitle}}; @else background:#337ab7; @endif padding:10px;">
	...
	<div style="background:#ffffff;border:solid 1px #005c73;padding:10px;">
		...
		Chúc mừng bạn đã đăng ký thành công <strong>{!!$messageArray->domain->domain!!}</strong></p> 
		<p>Tên miền đăng ký: <a href="{{route('channel.home',$messageArray->domain->domain)}}">{!!$messageArray->domain->domain!!}</a></p> 
		<p><strong>Ngày đăng ký:</strong> {!!$messageArray->domain->updated_at!!}</p>
		<p><strong>Ngày hết hạn:</strong> {!!$messageArray->domain->date_end!!}</p>
		<p><strong>Vào lúc:</strong> {!!$messageArray->domain->created_at!!}</p> 
