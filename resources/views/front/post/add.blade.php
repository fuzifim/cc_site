                    ...
                    <div class="panel">
                        <div class="panel-body">
                            <div id="post-form-container" class="col-sm-12 col-md-12 col-lg-12">
                                <form id="post-item-form" action="{{ route('front.post.save',$post->id) }}" method="post" accept-charset="utf-8" role="form" class="">
                                    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                                    <input type="hidden" name="post_status" value="{{ $post->post_status }}" />
                                    <input type="hidden" name="post_id" value="{{ $post->id }}"/>
									<input type="hidden" name="thumbnail" value="{{ $post->post_thumbnail }}"/>
									<div class="insert-media"></div>
									<div class="row">
										<div class="col-md-8 col-xs-12">
											<div class="add_message_summernote"></div>
											<div class="form-group">
												<div id="textTitle" style="font-weight:bold; " contentEditable="true" placeholder="Tiêu đề bài viết...">{{ $post->post_title }}</div>
												<input id="item-title" name="post_title" value="{{ $post->post_title }}" type="hidden" class="form-control title-post-edit" placeholder="Nhập tiêu đề bài đăng">
												<script>
											...
											<div class="form-group mrbt-5">
												<textarea name="post_description" id="summernote" rows="10" class="form-control summernote">{{ htmlspecialchars_decode($post->post_description) }}</textarea>
											</div>
											<div class="form-group">
												<div class="addMediaToContent">
													<? $i=0; ?>
													@if(count($postMedia)>0)
														@foreach($postMedia as $media)
															...
															@if ($media->file_type == "image/jpeg" || $media->file_type == "image/jpg" || $media->file_type == "image/png" || $media->file_type == "image/gif")
																<div class="item @if($i==1) active @endif col-xs-12 col-sm-6 col-md-3"><div class="mediaGallery"><img class="img-responsive" src="{{$media->file_url}}"></div> <div class="content">{{$media->file_content}}</div></div>
															@endif
															@if ($media->file_type == "video/x-flv" || $media->file_type == "video/mp4" || $media->file_type == "application/x-mpegURL" || $media->file_type == "video/MP2T" || $media->file_type == "video/3gpp" || $media->file_type == "video/quicktime" || $media->file_type == "video/x-quicktime" || $media->file_type == "image/mov" || $media->file_type == "video/avi" || $media->file_type == "video/x-msvideo" || $media->file_type == "video/x-ms-wmv")
																<div class="item @if($i==1) active @endif col-xs-12 col-sm-6 col-md-3"><div class="embed-responsive embed-responsive-16by9 mediaGallery"><iframe class="embed-responsive-item" src="{{$media->file_url}}" frameborder="0" allowfullscreen></iframe></div> <div class="content">{{$media->file_content}}</div></div>
															@endif
														@endforeach
													@endif 
												</div>
										...
										<div class="col-md-4 col-xs-12">
											<div class="form-group">
												<ul class="list-group">
													...
													@else
													<button type="button" tabindex="500" class="list-group-item btn-file" style="position: relative; "><i class="glyphicon glyphicon-camera"></i>&nbsp;  <span>Thêm ảnh/ Video</span><input id="mediaFile" name="input4[]" type="file" multiple="" class=""></button>
													@endif
													<button type="button" id="addChannel" class="list-group-item"><i class="glyphicon glyphicon-plus-sign"></i> Chọn kênh đăng</button>
											...
											<div class="form-group">
											 ...
											 <div id="category_option_product_seleted" class="container_product clear" @if(isset($post) && $post->category_option_post_id !=2)style="display:none;" @endif>
												<div class="col-md-6 col-sm-6 col-xs-12">
													<div class="form-group">
														<label class="control-label" for="item-price">Giá bán <span class="text-danger">(*)</span></label>
														@if (isset($post) && $post->post_price != 0)
														<input value="{{ $post->post_price }}" name="post_price" type="text" class="form-control" id="post-price"
														 data-placement="bottom" title=""
														placeholder="000" >
														@else
														<input value="" name="post_price" type="text" class="form-control" id="post-price"
														 data-placement="bottom" title=""
														placeholder="000">
														@endif
												...
												<div class="col-md-6 col-sm-6 col-xs-12">
													<div class="form-group">
														...
														@if (isset($post) && $post->discount != 0)
														<input value="{{ $post->discount }}" name="discount" type="text" class="form-control" id="discount"
														 data-placement="bottom" title=""
														placeholder="000" >
														@else
											...
											<div class="form-group tag_container_value_set" @if(!empty($tags) && count($tags)>0) style="display:block;" @else  style="display:none;" @endif>
												...
												<div id="tag_post" class="list-group">
													@if(!empty($tags) && count($tags)>0)
														@foreach($tags as $tag)
															<div class="list-group-item"><i class="fa fa-tag"></i> {{$tag->tag_name}} <span class="btn btn-xs btn-danger" onclick="delete_tag('{{$tag->tag_slug}}',{{$post->id}})" data="{{$tag->tag_slug}}"><i class="glyphicon glyphicon-trash"></i></span></div>
														@endforeach
													@endif
												</div>
											</div>
											<div class="form-group">
												...
												<a class="btn btn-danger" href="{{ route('front.post.delete',$post->post_id) }}"><i class="glyphicon glyphicon-trash"></i> Xóa</a> 
											</div>
										</div>
									</div>
...
<script>
	...
	$( "#addChannel").click(function() {
		...
		$.ajax({
			url: "{{route('front.post.join.channel')}}",
			type: 'post',
			cache: false,
			contentType: false,
			processData: false,
	...
	$( "#btnShowMedia" ).click(function() {
		...
		$.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name=_token]').attr('content')} }); 
		$.ajax({
			url: "{{route('front.media.get_media',array('post',$post->post_id))}}",
			type: 'GET',
			cache: false,
			contentType: false,
			processData: false,
			...
			success:function(result){
				$.each(result.medias, function(i, item) {
					...
					if(fileType=='image'){
						$('.addMediaContent').append('<div class="result-add-media"><div class="form-group"><button type="button" data-id="'+item.media_join_id+'" data-media-id="'+item.id+'" class="btn btn-xs btn-default btn-delMedia"><i class="glyphicon glyphicon-trash"></i></button> <img class="img-responsive item-media" data-type="image" data-id="'+item.id+'" src="'+item.file_url+'"></div><div class="form-group"><div id="contentBoxMedia" contentEditable="true" placeholder="Mô tả nội dung này...">'+item.file_content+'</div></div></div>'); 
					}else if(fileType=='video'){
						$('.addMediaContent').append('<div class="result-add-media"><div class="form-group"><button type="button" data-id="'+item.media_join_id+'" data-media-id="'+item.id+'" class="btn btn-xs btn-default btn-delMedia"><i class="glyphicon glyphicon-trash"></i></button><div class="embed-responsive embed-responsive-16by9"><iframe class="embed-responsive-item item-media" data-type="video" data-id="'+item.id+'" src="'+item.file_url+'" frameborder="0" allowfullscreen></iframe></div></div><div class="form-group"><div id="contentBoxMedia" contentEditable="true" placeholder="Mô tả nội dung này...">'+item.file_content+'</div></div></div>'); 
					}
				});
			},
			error: function(result) {
	...
	$('.addMediaContent').on('click', '.btn-delMedia', function() {
		...
		$.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name=_token]').attr('content')} }); 
		$.ajax({
			url: "{{route('home')}}/del_media_join/"+id,
			type: 'GET',
			cache: false,
			contentType: false,
			processData: false,
	...
	$( "#btnAddMedia" ).click(function() {
		$( ".result-add-media" ).each(function() {
			...
			$.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name=_token]').attr('content')} }); 
			$.ajax({
				url: "{{route('front.media.update_media')}}",
				type: 'post',
				cache: false,
				contentType: false,
				processData: false,
			...
			$.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name=_token]').attr('content')} }); 
			$.ajax({
				url: "{{route('front.media.media_join')}}",
				type: 'post',
				cache: false,
				contentType: false,
				processData: false,
				...
				success:function(result){
					if(type==='image'){
						$('.addMediaToContent').append('<div class="item col-xs-12 col-sm-6 col-md-3"><div class="mediaGallery"><img class="img-responsive" src="'+media+'"></div> <div class="content">'+content+'</div></div>'); 
					}else if(type==='video'){
						$('.addMediaToContent').append('<div class="item col-xs-12 col-sm-6 col-md-3"><div class="embed-responsive embed-responsive-16by9 mediaGallery"><iframe class="embed-responsive-item" src="'+media+'" frameborder="0" allowfullscreen></iframe></div> <div class="content">'+content+'</div></div>'); 
					}
				},
				error: function(result) {
				}
	...
	$('#mediaFile').bind("change", function(){ 
		...
		if(totalFile>0){
			...
			for(var i=0;i<totalFile;i++)
			{
				...
				var titlePost=$('input[name=post_title]').val(); 
				var formData = new FormData();
				formData.append("title", titlePost); 
				formData.append("file", files[i]); 
				$.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name=_token]').attr('content')} }); 
				$.ajax({
					url: "{{route('file.upload.note')}}",
					type: 'post',
					xhr: function() {
...
$(document).ready(function() { 
	...
	function uploadImage(file,editor,welEditable) {
		...
		var titlePost=$('input[name=post_title]').val(); 
		var description=$('textarea[name=post_description]').val();
