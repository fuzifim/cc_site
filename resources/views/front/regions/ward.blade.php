...
@section('seo')
<?php $data_seo = array(
         'title' => 'Doanh nghiệp tại '.$wards->ward_name.config('app.title_seo'),
         'keywords' => '',
         'description' => 'Thông tin các doanh nghiệp tại '.$wards->ward_name." - ".$districts->district_name." - ".$subregions->subregions_name." - ".$regions->country,
         'og_title' => 'Doanh nghiệp tại '.$wards->ward_name.config('app.title_seo'),
          'og_description' =>  'Thông tin các doanh nghiệp tại '.$wards->ward_name." - ".$districts->district_name." - ".$subregions->subregions_name." - ".$regions->country,
          'og_url' => '',
								...
								<div class="input-group" id="adv-search">
									<div class="input-group-btn">
										<div class="btn-group" role="group">
											<div class="dropdown dropdown-lg">
												<ol class="breadcrumb" itemscope="" itemtype="http://schema.org/BreadcrumbList">
													<li class="dropdown" itemprop="itemListElement">
													   ...
													   @if(count($regions_alls)>0)
														<ul class="dropdown-menu">
															  @foreach($regions_alls as $regions_list)
															  <li><a href="{{url('/')."/".$regions_list->iso}}/company">
													...
													<li class="dropdown hidden-xs" itemprop="itemListElement">
														<a class="btn btn-default dropdown-toggle btn-menu" itemprop="item" data-toggle="dropdown" href="{{url('/VN').$subregions->SolrID}}/company"><span itemprop="name">{{$subregions->subregions_name}}</span> <span class="glyphicon glyphicon-menu-down"></span></a>
														  @if(count($subregions_alls)>0)
																<ul class="dropdown-menu">
																   @foreach($subregions_alls as $subregions_all)
																   <li><a href="{{url('/').$subregions_all->SolrID}}/company">
													...
													<li class="dropdown hidden-xs" itemprop="itemListElement">
														<a class="btn btn-default dropdown-toggle btn-menu" data-toggle="dropdown" href="{{url('/VN').$districts->SolrID}}/company" itemprop="item"><span itemprop="name">{{$districts->district_name}}</span> <span class="glyphicon glyphicon-menu-down"></span></a>
														 @if(count($distric_cs)>0)
															  <ul class="dropdown-menu">
																  @foreach($distric_cs as $distric_c)
																	 <li><a href="{{url('/').$distric_c->SolrID}}/company">
													   ...
													   <li class="dropdown hidden-xs" itemprop="itemListElement">
														 <a class="btn btn-default dropdown-toggle btn-menu" data-toggle="dropdown" href="javascript:void(0)" itemprop="item"><span itemprop="name">Phường/Xã</span> <span class="glyphicon glyphicon-menu-down"></span></a>
														 @if(count($ward_cs)>0)
																<ul class="dropdown-menu">
																	 @foreach($ward_cs as $ward_c)
																	   <li><a href="{{url('/').$ward_c->SolrID}}/company">
													 ...
													 <li class="dropdown hidden-xs" itemprop="itemListElement">
														<a class="btn btn-default dropdown-toggle btn-menu" data-toggle="dropdown" href="{{url('/VN').$wards->SolrID}}/company" itemprop="item"><span itemprop="name">{{$wards->ward_name}}</span> <span class="glyphicon glyphicon-menu-down"></span></a>
														@if(count($ward_cs)>0)
															<ul class="dropdown-menu">
																 @foreach($ward_cs as $ward_c)
																   <li><a href="{{url('/').$ward_c->SolrID}}/company">
											...
											<input id="txt_search" type="text" class="form-control" placeholder="Doanh nghiệp: {{$wards->ward_name}}+{{$districts->district_name}}+{{$subregions->subregions_name}}+{{$regions->country}}" value="<?php if(isset($txt) && strlen($txt)>0){ echo $txt;}?>"/>
					...
					@foreach($customers as $data_customer)
						<div class="panel panel-default">
							<div class="panel-body panel-company">
								...
								@endif
								{{--<a href="{{route('front.customer.details',array($data_customer->id_auto,Str::slug($data_customer->customers_name)))}}"> {{$data_customer->customers_name}}</a>--}}
