											...
											<div class="item @if($i==0) active @endif row">
                                                      <div class="col-xs-12 col-sm-6 col-md-3  portfolio_utube_item">
														...
														@if(strlen($template->regionsID)>0 && strlen($template->subRegionsID)>0 )
                                                              <a href="{{route('front.ads.detail',array(App\Model\Regions::find($template->regionsID)->iso,App\Model\Subregions::find($template->subRegionsID)->subregions_name_slug,$template->title_shop_slug,$listProducts->ads_slug))}}" data-toggle="tooltip" data-placement="left" title="Hình ảnh">
