											...
											<div class="item @if($i==0) active @endif row">
                                                      <div class="col-xs-12 col-sm-6 col-md-3  portfolio_utube_item">
                                                           <div class="portfolio_utube_item_image">
															 ...
															 <a class="thumnail_tag" target="_top" href="{{route('front.ads.detail',array(App\Model\Regions::find($template->regionsID)->iso,App\Model\Subregions::find($template->subRegionsID)->subregions_name_slug,$template->title_shop_slug,$listProducts->ads_slug))}}" data-toggle="tooltip" data-placement="left">
                                                             <?php
                                                             $manager = new Intervention\Image\ImageManager(array('driver' => 'gd'));
                                                               $image = $manager->make(public_path($listProducts->ads_thumbnail))->resize(259, 160)->encode('jpg');
                                                               $type = 'jpg';
                                                             ...
                                                             @endif
                                                              ...
                                                              <a  class="icon_cat" href="{{route('front.ads.detail',array(App\Model\Regions::find($template->regionsID)->iso,App\Model\Subregions::find($template->subRegionsID)->subregions_name_slug,$template->title_shop_slug,$listProducts->ads_slug))}}" data-toggle="tooltip" data-placement="left">
                                                           ...
                                                           @if(strlen($template->regionsID)>0 && strlen($template->subRegionsID)>0 )
                                                               <a class="link_title_show" href="{{route('front.ads.detail',array(App\Model\Regions::find($template->regionsID)->iso,App\Model\Subregions::find($template->subRegionsID)->subregions_name_slug,$template->title_shop_slug,$listProducts->ads_slug))}}">{{$listProducts->ads_title}}</a>
                                                           @endif
